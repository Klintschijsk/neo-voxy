...
- 群系水色混合：四个版本都可在模型构建阶段平滑群系交界处的 LOD 水色；结果写入紧凑调色板，不进行逐 tick 世界遍历。
- LOD 构建压力：可在“最高帧数”到“最高追赶”之间调节每帧节点处理与模型烘焙预算。
- 圆形 LOD 淡入：四个版本统一使用三维相机距离和世界坐标稳定抖动，在原版区块与 LOD 之间交接；水体与树叶采用各自的无透明渐变交接策略。若光影自身已有 LOD 过渡（例如 Photon），应关闭此功能，避免双重过渡、噪点或阴影边界。
