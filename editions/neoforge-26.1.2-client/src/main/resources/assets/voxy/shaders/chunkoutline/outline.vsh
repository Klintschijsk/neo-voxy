...
void main() {
    ...
    ivec3 cornerSelector = ivec3(gl_VertexID&1, (gl_VertexID>>2)&1, (gl_VertexID>>1)&1);
    vec3 cubeCorner = vec3(cornerSelector*16);
    // Keep a sub-pixel guard band for LOD geometry at the vanilla boundary.
    // Expanding this mask removes already-culled LOD boundary faces; a small
    // inset instead closes terrain and fluid seams without adding geometry.
    const vec3 boundaryInset = vec3(0.0625, 0.03125, 0.0625);
    cubeCorner += mix(boundaryInset, -boundaryInset, vec3(cornerSelector));
