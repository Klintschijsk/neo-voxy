...
layout(location = 6) uniform float fogIntensity;
layout(location = 7) uniform float fogDensity;
layout(location = 8) uniform int linearFog;
#endif
#endif
...
void main() {
    ...
    #ifdef USE_ENV_FOG
    if (fogIntensity > 0.0){
        float linearAmount = clamp((length(point.xyz) - fogParams.x)
