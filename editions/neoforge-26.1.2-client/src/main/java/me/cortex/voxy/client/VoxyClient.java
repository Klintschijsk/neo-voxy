...
public class VoxyClient {
   ...
   public static void initVoxyClient() {
      ...
      if (systemSupported && System.getProperty("voxy.exclusiveLock", "false").equalsIgnoreCase("true")) {
         Path vf = Minecraft.getInstance().gameDirectory.toPath().resolve(".voxy");
         ...
         try {
            ...
            EXCLUSIVE_LOCK = fis.getChannel().lock(0L, Long.MAX_VALUE, false);
         } catch (IOException | NonWritableChannelException var3) {
   ...
   public static void initializeNeoForge(ModContainer container) {
      DebugEntries.init();
   }
