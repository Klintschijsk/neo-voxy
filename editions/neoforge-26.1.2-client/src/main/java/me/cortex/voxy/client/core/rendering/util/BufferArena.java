...
public class BufferArena {
   ...
   public BufferArena(long capacity, int elementSize) {
      ...
      } else if (CHECK_SSBO_MAX_SIZE_CHECK && capacity > Capabilities.INSTANCE.ssboMaxSize) {
         throw new IllegalArgumentException(
            "Buffer is bigger than max ssbo size (requested " + capacity + " but has max of " + Capabilities.INSTANCE.ssboMaxSize + ")"
         );
      } else {
         this.size = capacity;
   ...
   public long upload(MemoryBuffer buffer) {
      if (buffer.size % this.elementSize != 0L) {
   ...
   public void free(long allocation) {
      this.used = this.used - this.allocationMap.free(allocation);
   }
