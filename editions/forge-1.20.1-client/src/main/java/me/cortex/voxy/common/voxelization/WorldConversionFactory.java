...
public class WorldConversionFactory {
    ...
    private static void setupGenericLocalPalette(Palette<BlockState> palette,
    ...
    private static void setupLocalPalette(Palette<BlockState> vp, Reference2IntOpenHashMap<BlockState> blockCache, Mapper mapper, int[] pc) {
        if (vp instanceof LinearPalette<BlockState>) {
            ...
            // Use the public Palette API for third-party palette implementations. This path is only
            // reached for non-vanilla palettes and avoids a hard dependency on their implementation.
            setupGenericLocalPalette(vp, blockCache, mapper, pc);
        }
    }
    ...
    public static VoxelizedSection convert(VoxelizedSection section,
        ...
        if (blockContainer.data.storage instanceof SimpleBitStorage bStor) {
            ...
            for (int i = 0; i <= 0xFFF; i++) {
                ...
                data[i] = Mapper.composeMappingId(light, bId, biomes[me.cortex.voxy.common.util.BitUtils.compress(i,0b1100_1100_1100)]);
            }
        } else {
