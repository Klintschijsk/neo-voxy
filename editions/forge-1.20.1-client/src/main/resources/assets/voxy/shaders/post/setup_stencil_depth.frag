...
void main() {
    ...
    if (fadeEnabled && boundaryDistance > lodBoundaryFadeStart
            && boundaryDistance < lodBoundaryFadeEnd) {
        coverage = smoothstep(lodBoundaryFadeStart, lodBoundaryFadeEnd, boundaryDistance);
        coverage *= coverage;
    ...
    if (boundaryGuardPass != 0) {
        if (!fadeEnabled || boundaryDistance <= lodBoundaryFadeStart
                || boundaryDistance >= lodBoundaryFadeEnd || ditherValue >= coverage) discard;
        float rayLength = max(length(cameraRelative.xyz), 1.0);
        vec3 guardedPosition = cameraRelative.xyz * (1.0 + min(2.0 / rayLength, 0.125));
    ...
    if (fadeEnabled && (boundaryDistance >= lodBoundaryFadeEnd
            || (boundaryDistance > lodBoundaryFadeStart && ditherValue < coverage))) discard;
    gl_FragDepth = NEAR;
}
